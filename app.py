...
def classify_stock_groups(subject_col):
    now = time.time()
    cached = _classify_cache.get(subject_col)
    if cached is not None and (now - cached[0]) < 3600:
        return cached[1]
    result = _classify_stock_groups_impl(subject_col)
    _classify_cache[subject_col] = (now, result)
    return result

def _classify_stock_groups_impl(subject_col):
    if not os.path.exists("data/investor_data.csv"):
        return [], [], []
    try:
